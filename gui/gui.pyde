#defines the combat module
#clarification is needed by mark
def combat():
...
#draws the window and sets up variables

def setup():
    global font, diceRoll, state, mainMenu, combat
    import random
...
def draw():
    global state, mainMenu, combat
    #sets state to  mainMenu for untill start screen is implemented
    state = mainMenu
    
    #checks if combat button has been clicked, and draws accordingly
    
    if state == combat:
        combat()
    
   #checks if no modules have been activated and draws accordingly
