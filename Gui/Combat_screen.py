...
def corruption():
    ...
    if cards.corruptionIsPlayed == True:
        if turn == 'player':
...
def fireball():
    global fireballIsPlayed, fireCountPlayer, fireCountMonster
    if cards.fireballIsPlayed == True:
        if turn == 'player':
            fireCountMonster = 3
        if turn == 'monster':
            fireCountPlayer = 3
def turnCount():
    global turnCount
    if cards.corruptionIsPlayed == True or cards.poisonIsPlayed == True or cards.fireballIsPlayed:
        if endTurn == True:
            turnCounter =+ 1
...
def draw():
    ...
    if corruptionCountPlayer > 0:
        image(cor,920, 620)
    if fireCountPlayer > 0:
        image(brn, 920, 620)
        
    cards.dmgCalc(cards.selectedCard)
...
def doCalc():
    global hpPlayer, hpMonster, energyPlayer, energyMonster, blockPlayer, blockMonster
    if endTurn == True
        if cards.cardPlayed == 'poison' or cards.cardPlayed == 'corruption' or cards.cardPlayed == 'fireball':
            status = cards.cardPlayed
        cards.damageCalc(cards.cardPlayed)
